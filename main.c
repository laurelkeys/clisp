...
typedef enum { LERR_DIV_ZERO, LERR_BAD_OP, LERR_BAD_NUM } LERR_TYPE;
...
lval lval_num(const long x) {
    return (lval) { .type = LVAL_NUM, .num = x };
}

lval lval_err(const LERR_TYPE x) {
    return (lval) { .type = LVAL_ERR, .err = x };
}
